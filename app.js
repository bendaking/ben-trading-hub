...
function submitFlashcard() {
    ...
    const flashcard = {
        id: editingFlashcardId || Date.now().toString(),
        term: term,
        definition: def
    };
    ...
    if (editingFlashcardId) {
        ...
        if (idx >= 0) store.data.flashcards[idx] = flashcard;
    } else {
        store.data.flashcards.push(flashcard);
    }
...
function deleteFlashcard(id) {
    if (confirm('Delete this flashcard?')) {
        ...
        store.save();
        if (currentCardIndex >= store.data.flashcards.length) {
            currentCardIndex = Math.max(0, store.data.flashcards.length - 1);
        }
        renderFlashcards();
    }
}
...
function renderFlashcards() {
    ...
    container.innerHTML = '<div class="flashcard-wrapper" onclick="this.classList.toggle(\'flipped\')">' +
        ...
        '<button onclick="editFlashcard(\'' + c.id + '\')"><i class="fas fa-edit"></i> Edit</button>' +
        '<button onclick="deleteFlashcard(\'' + c.id + '\')"><i class="fas fa-trash"></i> Delete</button>' +
        '</div>';
}
